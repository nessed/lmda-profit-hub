    ...
    try {
      ...
      const enriched = await Promise.all(
        workshops.map(async (w) => {
          ...
          return { ...w, registrations: regs, latestSnapshot, revenue, profit, profitMargin };
        })
      );
      ...
      <div className="space-y-8">
        <header className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <div>
              ...
              <p className={cn("text-4xl sm:text-5xl font-bold tracking-tight font-mono", totalMonthlyProfit >= 0 ? "text-success" : "text-destructive")}>
                {formatCurrency(totalMonthlyProfit)}
              </p>
              {totalMonthlyRevenue > 0 && (
                <p className="text-sm text-muted-foreground mt-1">from {formatCurrency(totalMonthlyRevenue)} revenue</p>
              )}
            </div>
            {nextWorkshop && (
              <div className="text-right">
                <p className="text-sm text-muted-foreground">Next Workshop</p>
                <Link to={`/workshops/${nextWorkshop.id}`} className="text-lg font-medium text-foreground hover:text-primary transition-colors">
                  {nextWorkshop.title}
                </Link>
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/workshops" className="text-sm text-primary hover:underline font-medium">Manage</Link>
          </div>
          {workshopsData.length === 0 ? (
            <div className="bg-card border border-border rounded-xl p-8 text-center">
              <p className="text-muted-foreground">No workshops yet.</p>
              <Link to="/workshops" className="text-primary hover:underline text-sm">Create your first workshop</Link>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {workshopsData.map((workshop) => (
                <Link key={workshop.id} to={`/workshops/${workshop.id}`} className={cn("profit-card group hover:scale-[1.02] transition-transform cursor-pointer", getProfitCardClass(workshop.profit))}>
                  <div className="space-y-3">
                    <div>
                      <h3 className="font-semibold text-foreground group-hover:text-primary transition-colors truncate">{workshop.title}</h3>
                      <p className="text-sm text-muted-foreground">{formatDate(workshop.date)}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={cn("text-2xl sm:text-3xl font-bold font-mono", workshop.profit >= 0 ? "text-success" : "text-destructive")}>
                        {formatCurrency(workshop.profit)}
                      </span>
                      {workshop.latestSnapshot && <span className={cn("health-dot", getHealthClass(workshop.profitMargin))} />}
                    </div>
                    <div className="space-y-1">
                      <div className="margin-bar">
                        <div className="margin-bar-fill" style={{ width: `${Math.max(0, Math.min(100, workshop.profitMargin))}%` }} />
                      </div>
